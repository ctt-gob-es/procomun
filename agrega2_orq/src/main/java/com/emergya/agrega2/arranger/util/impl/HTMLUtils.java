...
import com.emergya.agrega2.arranger.util.Content;
...
public class HTMLUtils {
    ...
    public static Content getContent(final URL urlObject, final String initDelimiter, final String endDelimiter, final boolean followRedirects, final Charset cs) {

        if (urlObject != null) {
            try {
                // TODO SET TIMEOUT BY DEFAULT TO 2000ms
                ...
                if (conn instanceof HttpURLConnection) {
                    ...
                    if (contentType.startsWith("text/") || contentType.startsWith("application/xml")) {
                        final String stringContent = readContentAsString(responseCode == HttpURLConnection.HTTP_OK ? conn.getInputStream() : httpConn.getErrorStream(), charsetName,
                                initDelimiter, endDelimiter);
    ...
    private static String readContentAsString(final InputStream is, final String charsetName, final String initDelimiter, final String endDelimiter) throws IOException {
    ...
    private static Content createContent(final String contentType, final String charsetName, final String stringContent, final int responseCode) {
        return new Content() {
