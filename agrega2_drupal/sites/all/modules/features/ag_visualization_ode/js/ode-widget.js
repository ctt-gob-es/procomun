/**
 * This widget renders the ODE Visualizer on external pages
 */
var env = env || 'prod';
try {
  (function () {
    ...
    /* Find the viewers placeholders and create the iframes for rendering the visualizer */
    var ode_viewer_placeholders = Sizzle(".agrega2-ode-viewer");

    var script_url = document.getElementById("ode-widget").src;

    var matches = script_url.match(/^(https?\:\/\/[^\/?#]+)(?:[\/?#]|$)/i);
    var host = matches && matches[1];

    if (host && ode_viewer_placeholders.length) {
      for(var i = 0; i < ode_viewer_placeholders.length; i++) {
        var holder = ode_viewer_placeholders[i];

        var ode_id = holder.getAttribute('data-ode');
        ...
        if (typeof Drupal == "undefined") {
          var url = host + '/' + 'ode/view/' + ode_id + '/widget';
        }
        else {
          var url = host + '/' + Drupal.settings.pathPrefix + 'ode/view/' + ode_id + '/widget';
        }
        var iframe = document.createElement('iframe');
        iframe.setAttribute('frameborder', '0');
        iframe.setAttribute('height', '657px');
    ...
    /* Communicate with the iframe for adapting sizes */
    window.AGODEWidgetController || (function (window) {

      window.AGODEWidgetController = {
        ...
        handleMessage: function (event) {
          ...
          if (event.data == 'init') {
            ...
            if (env == 'dev') {
              console.log('Parent Visualizer iFrame registered');
            }

            $this.visualizerIframe.postMessage("init_parent", $this.visualizerOrigin);
            return;
          }
          ...
          if (message) {
            switch (message.type) {
              case 'setHeight':
                /* @todo we need to know on which item set the height,
                 *        currently we just support one element per page
                 */
                var ode_viewer_placeholders = Sizzle(".agrega2-ode-viewer");

                if (ode_viewer_placeholders.length) {
                  for(var i = 0; i < ode_viewer_placeholders.length; i++) {
                    var style = ode_viewer_placeholders[i].style;
                    style.height = message.value + 'px';
              ...
              case 'setIframeHeight':
                 ...
                 *        currently we just support one element per page
                 */
                var iframes = Sizzle(".agrega2-ode-viewer iframe");

                if (iframes.length) {
                  for(var i = 0; i < iframes.length; i++) {
                    var style = iframes[i].style;
                    style.height = message.value + 'px';
                  }
                }
                break;
      ...
      var $this = window.AGODEWidgetController;

      // useCapture might be true
      window.addEventListener("message", window.AGODEWidgetController.handleMessage, false);
