...
Nombre del parche: search_api_solr_several_errors.patch
Descripción: parche que actualiza el módulo para que funcione correctamente con el módulo sarnia

NAME: search_api_solr_autocomplete_add_facet_fields_name.patch
DESCRIPTION: Add a new level when building the autocomplete array because we need to print the facet
 field name to understand what filter to apply(autosubmit process).

NAME: search_api_solr_strtolower_user_input.patch
DESCRIPTION: Check the user input and avoid terms not containing it.

NAME: search_api_solr_facet_prefix_alter.patch
DESCRIPTION: We have a special way to build the autocomplete query. I.E.

 "facet":"true",
       "indent":"true",
       "facet.mincount":"1",
       "q":"(userLabelsAuto:matemáti OR generalKeywordAuto:matemáti OR generalTitleAuto:matemáti OR userNameSurnameAuto:matemáti)",
       "facet.limit":"50",
       "facet.field":["userNameSurnameStr",
         "userLabelsStr",
         "generalKeywordStr",
         "generalTitleStr"],
       "wt":"json",
       "fq":"type:(ODE OR LEARNING_RESOURCE OR USER)",
       "rows":"0"

 With this patch we are using facet.prefix as the user input to help us to build this kind of query.