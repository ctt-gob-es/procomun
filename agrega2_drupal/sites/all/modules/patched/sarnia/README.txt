...
Nombre del fichero: sarnia_several_errors.patch
Descripción: parche inicial para hacer que el módulo funcione, ya que estaba desactualizado y había conflictos con el Search API

Nombre del fichero: sarnia_let_displays_facets_to_make_facets_renderables.patch
Descripción: Cuando el modulo sarnia se indexa en un core, este crea una entidad de indexado a la cual por defecto no se le crea ningun Display 
             Widget, ni ningun view mode, por este motivo se nos presenta la situación de que podemos crear facetas y sus respectivos bloques,
             pero dichas facetas no serán renderizadas en ningun caso puesto que la entidad posee un termino que no permite los actuales 
	     displays widgets, dicho termino es "sarnia_term", el cual se creo en primera instancia para poder controlar y adaptarse de forma
	     mucho más eficiente a las query realizadas por solr, pero dicha adaptación no está conseguida. 
	     Lease : https://drupal.org/node/1950256


Nombre del fichero: sarnia_preprocess_modifications.patch
Descripción: Modificado el preprocess del módulo Sarnia para renderizar views utilizando diferentes tpls (ode y user) al realizar una búsqueda.

NAME:sarnia_avoid_request_every_field.patch
Description: Avoid to request every field per query.

NAME:sarnia_number_results_filter_block_top.patch
Description: Adding pager and number of results to the tpl views-view--ode-search--page.tpl.php because we need to show the number of results per page and the total number of results when filtering by a facet.

NAME: sarnia_change_name_first_result_position_var.patch
Description: Change $current_page variable with a more intuitive name($first_result_position).

NAME: sarnia_remove_dismaxx_solr_query.patch
Description: Removed qt=dismax from query as required.