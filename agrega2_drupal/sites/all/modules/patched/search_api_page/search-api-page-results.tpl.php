...
<div class="<?php print $classes;?>">
  ...
  <?php
    foreach($search_results as $key => $value){
      print_r ("resultado nº:  ");
      print_r($key);

      foreach (($value['#result']) as $key_field => $value_field) {
        if ($key_field == 'fields'){
            foreach ($value_field as $solr_field_key => $solr_field_value) {
              if ($solr_field_key == 'id'){
                print l(t($solr_field_key.' -->  '.$solr_field_value),'ode/'.$solr_field_value, array('attributes' => array('id' => $solr_field_value, 'class' => 'refresh-link'), 'html' => FALSE));
                print_r('<br/>');
              }
              else{
              print_r($solr_field_key);
              print_r("   --->   ");
              print_r($solr_field_value);
              print_r("<br/>");
              }
            }
        }
