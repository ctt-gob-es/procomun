...
// Localize jQuery variable
  ...
  function drupalchatMain() {
    ...
    jQuery(document).ready(function () {
      ...
      jQuery("#mainpanel li a.subpanel_toggle, .chatbox a.chatboxhead").live('click', function (e) { //If clicked on the first link of #chatpanel...

        var $currentTarget = jQuery(e.currentTarget);
        if ($currentTarget.hasClass('chatboxblink')) {
          $currentTarget.removeClass('chatboxblink');
        }
    ...
    function createChatBox(chatboxtitle, chatboxname, chatboxblink) {
      ...
      jQuery.getJSON(Drupal.settings.drupalchat.logMessagesUrl, function (data) {
        ...
        jQuery.each(data.messages, function (key, msg) {

          // Heading data of the message
          var output =
            '<div class="chatboxusername">' +
              '<div class="chatboxuserpicture">' +
              '<img height="20" width="20" src="' + msg.user_image + '">' +
              '</div>' +
              '<span class="chatboxtime">' + msg.date + '</span>' +
              '<a href="/user/' + msg.uid + '">' + msg.user + '</a>' +
              '</div>';
    ...
    function processChatData(data) {
      ...
      if ((!drupalchat_messages.status || drupalchat_messages.status == 0)) {
        ...
        jQuery.each(drupalchat_messages.messages, function (index, value) {
          ...
          var has_chatboxblink = jQuery("#chatbox_" + chatboxtitle + ' .chatboxhead').hasClass('chatboxblink');

          if (jQuery("#chatbox_" + chatboxtitle).length <= 0 && !has_chatboxblink) {
            ...
            // Chat is minimized and has not chatboxblink. We add the class to notify the user.
            if(!has_chatboxblink){
              jQuery("#chatbox_" + chatboxtitle + ' .chatboxhead').addClass('chatboxblink');
            }
